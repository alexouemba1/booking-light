...
function cleanQueryParams(router: any, search: URLSearchParams) {
...
function removeFocusParam(router: any, search: URLSearchParams) {
  const next = new URLSearchParams(search.toString());
  next.delete("focus");
  const qs = next.toString();
  router.replace(qs ? `?${qs}` : "?", { scroll: false });
...
export default function MyBookingsPage() {
  ...
  // ✅ Map booking_id -> review_id (pour bouton "Modifier mon avis")
  ...
  async function loadData() {
    ...
    if (lErr) {
      console.warn("[my-bookings] listings load error:", lErr.message);
    } else {
    ...
    // ✅ Charger reviews existants pour ces bookings (1 requête)
    ...
    if (rErr) {
      // pas bloquant
      console.warn("[my-bookings] reviews load error:", rErr.message);
      setReviewIdByBookingId({});
    } else {
  ...
  async function fetchBookingState(bookingId: string) {
    ...
    const pay = String((data as any).payment_status || "").toLowerCase();
      ...
      if (paid === "1") {
        ...
        if (bookingIdFromUrl) {
          ...
          for (let i = 0; i < maxTries && alive; i++) {
            ...
            const synced =
              !!st &&
              (st.pay === "paid" || st.status === "confirmed" || st.status === "paid" || st.expiresAt === null);
      ...
      {items.length === 0 ? (
        ...
        <div style={{ display: "grid", gap: 12 }}>
          {items.map((b) => {
            ...
            // Avis éligible si séjour fini + payé/confirmé (et pas pending)
