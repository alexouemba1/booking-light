...
type ConnectStatus =
  | { hasAccount: false; active: false; mode?: "test" | "live"; message?: string }
  | {
      hasAccount: true;
...
function statusHelp(st: ConnectStatus | null) {
  ...
  if (!st.hasAccount) return "Activez Stripe Connect pour pouvoir recevoir des paiements automatiquement.";
  if (st.active) return "Votre compte est prêt : les paiements peuvent être encaissés et reversés au loueur.";
    ...
    <main className="bl-container">
      ...
      >
        <div style={{ display: "flex", gap: 10, alignItems: "center", justifyContent: "space-between", flexWrap: "wrap" }}>
          ...
          <button className="bl-btn" onClick={fetchStatus} disabled={checking || loading} style={{ marginTop: 0, height: 40, fontWeight: 900 }}>
            {checking ? "…" : "Actualiser"}
          </button>
        </div>
        ...
        {status?.mode && (
          <div style={{ marginTop: 6, fontSize: 12, opacity: 0.75, fontWeight: 800 }}>
            Mode : <span style={{ fontFamily: "ui-monospace, SFMono-Regular, Menlo, monospace" }}>{status.mode}</span>
          </div>
        )}
        ...
        {status && status.hasAccount && (
          <div style={{ marginTop: 8, fontSize: 12, opacity: 0.75, fontWeight: 800 }}>
            Compte Stripe :{" "}
            <span style={{ fontFamily: "ui-monospace, SFMono-Regular, Menlo, monospace" }}>{status.stripeAccountId}</span>
            <br />
      ...
      {!isActive ? (
        <button className="bl-btn bl-btn-primary" onClick={startStripeOnboarding} disabled={loading || checking} style={{ marginTop: 14 }}>
          {loading ? "Redirection…" : "Activer les paiements (Stripe Connect)"}
        </button>
      ) : (
        <div style={{ marginTop: 14, fontWeight: 900, opacity: 0.85 }}>✅ Tout est prêt. Vous pouvez recevoir des paiements.</div>
      )}
